...
class SiteMessageFactory(factory.django.DjangoModelFactory):
    ...
    display_start = factory.Faker("future_datetime", tzinfo=timezone.get_current_timezone())
    event_start = factory.Faker("future_datetime", tzinfo=timezone.get_current_timezone())
    event_end = factory.Faker("future_datetime", tzinfo=timezone.get_current_timezone())
    creator = factory.SubFactory(UserFactory)
    type = factory.Faker("random_element", elements=[x[0] for x in Message.Type.choices])
    dismissal_policy = factory.Faker("random_element", elements=[x[0] for x in Message.Policy.choices])
...
def create_site_message(display_start, event_start, event_end, id):
    ...
    """
    message = SiteMessageFactory(display_start=display_start, event_start=event_start, event_end=event_end, id=id)
    return message