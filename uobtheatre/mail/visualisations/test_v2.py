...
def test_booking_conf_new():
    ...
    test_mail = MailComposer.blank([
        ...
        Paragraph(title="Your Tickets", titleIcon=icons["ticket"]),

        BoxCols([
            RowStack([
                Paragraph(title="Ticket 1"),
                QR(f"[\"{booking.reference}\",\"ticketId1\"]")
            ]),
            RowStack([
                Paragraph(title="Ticket 2"),
                QR(f"[\"{booking.reference}\",\"ticketId2\"]")
            ]),
            RowStack([
                Paragraph(title="Ticket 3"),
                QR(f"[\"{booking.reference}\",\"ticketId3\"]")
            ])
        ]),

        BoxCols([RowStack([
                Paragraph(title="Ticket 4"),
                QR(f"[\"{booking.reference}\",\"ticketId4\"]")
                ])])
    ]
    )
