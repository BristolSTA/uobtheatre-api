...
def test_heading():
    heading = Heading(title="Title", subtitle="Sub", subsubtitle="SubSub", message="Msg", titleIcon="clock", messageIcon="search")
...
def test_timings_block():
    ...
    doors_str = perf.doors_open.astimezone(perf.venue.address.timezone).strftime('%A, %d %B %Y at %H:%M (%Z)')
    start_str = perf.start.astimezone(perf.venue.address.timezone).strftime('%A, %d %B %Y at %H:%M (%Z)')
    text = tb.to_text()
...
def test_sub_items_recursive():
    ...
    row = RowStack([col])
    all_items = ComposerItemInterface.collect_sub_items(row)
    assert para in all_items and btn in all_items and col in all_items and row in all_items
